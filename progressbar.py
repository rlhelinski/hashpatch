...
class ETA(ProgressBarWidget):
    ...
    def update(self, pbar):
        if pbar.numer == 0:
            ...
            eta = elapsed * pbar.denom / pbar.numer - elapsed
...
class FileTransferSpeed(ProgressBarWidget):
    ...
    def update(self, pbar):
        if pbar.seconds_elapsed < 2e-6:#== 0:
            ...
            bps = float(pbar.numer) / pbar.seconds_elapsed
        spd = bps
        for u in self.units:
            if spd < 1000:
                break
...
class Fraction(ProgressBarWidget):
	"Show a numerator and denominator"
	def __init__(self, human=False):
		self.human = human
	def update(self, pbar):
		if (self.human):
			return "%s / %s" % (humanize_bytes(pbar.numer), humanize_bytes(pbar.denom))
		else:
			return "%d/%d" % (pbar.numer, pbar.denom)
...
class ProgressBar(object):
    ...
    def percentage(self):
        ...
        return self.numer*100.0 / self.denom
	...
	#print "%d" % (datetime.datetime.now() - self.last_update).microseconds
        return (int(self.percentage()) != int(self.prev_percentage)) and \
    ...
    def update(self, numer=1, denom=False):
		...
		self.denom = denom
	self.numer = numer
	assert 0 <= self.numer <= self.denom
    ...
    def finish(self):
        ...
        self.update(self.denom, self.denom)
        if self.signal_set:
